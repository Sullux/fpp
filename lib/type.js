...
const assertInstance = (c) =>
  (v) => {
    ...
    throw new Error(`compile error: expected ${c.name}; got ${typeName(v)}`)
  }

const ref = fpp.ref || (fpp.ref = Symbol('fpp.ref'))

const Ref = (name) => ({ [ref]: name })
Object.defineProperty(Ref, Symbol.hasInstance, { value: (v) => !!v?.[ref] })
const Expr = (terms, foo) => {} // todo
...
const Spec = (def) => {
  ...
  const existing = specs.get(def) || specs.get()
  if ((typeof def) === 'function') {
    //
  }
}
Object.defineProperties(Spec, {
  type: { value: (name) => Spec((v) => (typeof v) === name) },
  value: { value: (expected) => Spec((v) => v === expected) },
  property: { value: (name, value) => Spec((v) => v?.[name] === value) },
})
...
const Type = (...specs) => {
  ...
  specs.map(assertInstance(Spec))
  return Object.freeze({
    [fppType]: specs,
    [Symbol.hasInstance]: (v) => specs.some((spec) => spec(v)),
  })
}
